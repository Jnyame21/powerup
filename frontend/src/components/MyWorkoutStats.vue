...
const groupedWorkoutData = computed(()=>{
  ...
  userAuthStore.workoutTypes?.forEach(item=> {
    workoutData[item.name] = []
  })
...
<template>
  <div class="content-wrapper" v-show="elementsStore.activePage === `MyWorkoutStats${props.typeValue},${props.keyValue}`" :class="{ 'is-active-page': elementsStore.activePage === `MyWorkoutStats${props.typeValue},${props.keyValue}` }">
    ...
    <NoData v-if="props.data.length === 0" />
      ...
      <v-row dense>
        <v-col v-for="[key, value] in Object.entries(groupedWorkoutData)" :key="key" cols="12" sm="6" md="6" lg="4">
          <v-card class="workout-card" elevation="1" rounded="lg">
            ...
            <v-card-title class="card-title">
              POINTS EARNED <v-icon icon="mdi-star" color="yellow" size="small"/>: <v-chip color="blue" size="small">{{ Number(value.map(item=> item.points).reduce((total, num)=> total + Number(num), 0)) }}</v-chip>
            </v-card-title>
            <v-card-title class="card-title">
              CALORIES BURNED <v-icon icon="mdi-fire" color="red" size="small"/>: <v-chip color="blue" size="small">{{ Number(value.map(item=> item.calories_burned).reduce((total, num)=> total + Number(num), 0)) }}</v-chip>
            </v-card-title>
          </v-card>
        </v-col>
      </v-row>
